...
|---|---|---|---|
| 2026-03-30 | Replaced boundary-signature `Vec<i8>` keys with compact `u128` boundary keys in Z/W bucketing. | Removes frequent small-vector allocations and reduces hash-key size in candidate bucket maps. | ~24.28% mean improvement and ~27.01% median improvement vs immediate prior run (`6.383 -> 4.833` mean, `6.335 -> 4.624` median). |
