...
| 2026-03-31 | Binary implication graph in radical: store binary clauses (a ∨ b) as implication pairs (¬a→b, ¬b→a) with clause index. Propagate binary implications before 2WL for longer clauses. | Binary clauses are ~40% of the TT encoding (from XNOR and totalizer). Direct implication propagation skips the 2WL overhead (no watch list scanning, no literal swapping). Also improves conflict analysis by catching implications earlier. | Full SAT n=16: `4.3s → 2.1s` (**-51%**), conflicts 56K→23K. |
