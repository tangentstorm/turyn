...
- `docs/TTC-AUDIT.md`: **Read before trusting any `covered` / TTC number.** Audit of coverage honesty per mode + §12 on what was fixed. Includes `check_coverage`, which validates a run against the published catalogue — use it after any change that could affect what the search covers.
