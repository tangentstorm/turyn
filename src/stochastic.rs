...
pub(crate) fn stochastic_search(
    ...
    if verbose {
        println!(
            "TT({}): stochastic search with {} threads",
            problem.n, workers
        );
    }
...
pub(crate) fn stochastic_worker(
    problem: Problem,
    norm: &[SumTuple],
    found: &AtomicBool,
    seed: u64,
    ...
    let fix_sum = |seq: &mut [i8], target: i32, rng: &mut dyn FnMut() -> u64, start: usize| {
        ...
        loop {
            ...
            if current < target && seq[idx] == -1 {
                seq[idx] = 1;
            } else if current > target && seq[idx] == 1 {
                seq[idx] = -1;
            }
        }
    };
    ...
    for restart in 0..max_restarts {
        ...
        if defect == 0 {
            ...
            let ok = verify_tt(
                problem,
                &PackedSeq::from_values(&x),
                &PackedSeq::from_values(&y),
                &PackedSeq::from_values(&z),
                &PackedSeq::from_values(&w),
            );
            return SearchReport {
                stats,
                elapsed: run_start.elapsed(),
        ...
        let mut pos_idx: [Vec<Vec<usize>>; 4] = [
            vec![vec![], vec![]],
            vec![vec![], vec![]],
            vec![vec![], vec![]],
            vec![vec![], vec![]],
        ...
        for flip in 0..max_flips {
            ...
            for s in 1..n {
                ...
                let mut nb = 0i32;
                if p + s < seq_len && p + s != q {
                    nb += seq[p + s] as i32;
                }
                if p >= s && p - s != q {
                    nb += seq[p - s] as i32;
                }
                if q + s < seq_len && q + s != p {
                    nb += seq[q + s] as i32;
                }
                if q >= s && q - s != p {
                    nb += seq[q - s] as i32;
            ...
            let accept = if delta <= 0 {
                ...
                (rng() % 10000) as f64 / 10000.0 < (-delta as f64 / temp).exp()
            } else {
                false
            };
            if accept {
                ...
                if defect == 0 {
                    ...
                    if verbose {
                        print_solution(
                            &format!(
                                "TT({}) found (restart {} flip {})",
                                problem.n, restart, flip
                            ),
                            &px,
                            &py,
                            &pz,
...
impl SatEncoder {
    pub(crate) fn new(n: usize) -> Self {
        ...
        Self {
            n,
            next_var: (3 * n + m + 1) as i32,
            xnor_triples: Vec::new(),
        }
    }

    pub(crate) fn x_var(&self, i: usize) -> i32 {
        (i + 1) as i32
    }
    pub(crate) fn y_var(&self, i: usize) -> i32 {
        (self.n + i + 1) as i32
    }
    pub(crate) fn z_var(&self, i: usize) -> i32 {
        (2 * self.n + i + 1) as i32
    }
    pub(crate) fn w_var(&self, i: usize) -> i32 {
        (3 * self.n + i + 1) as i32
    }
    ...
    pub(crate) fn build_counter(&mut self, solver: &mut impl SatSolver, lits: &[i32]) -> Vec<i32> {
        let n = lits.len();
        if n == 0 {
            return vec![0];
        }
        ...
        for a in 0..=left_max {
            for b in 0..=right_max {
                ...
                match (l, r) {
                    ...
                    (0, _) => {
                        solver.add_clause([-r, out[c]]);
                    }
                    (_, 0) => {
                        solver.add_clause([-l, out[c]]);
                    }
                    (_, _) => {
                        solver.add_clause([-l, -r, out[c]]);
                    }
                }
            }
        }
        ...
        for a in 0..=left_max {
            for b in 0..=right_max {
                ...
                match (l_next, r_next) {
                    ...
                    (0, _) => {
                        solver.add_clause([-out[c], r_next]);
                    }
                    (_, 0) => {
                        solver.add_clause([-out[c], l_next]);
                    }
                    (_, _) => {
                        solver.add_clause([-out[c], l_next, r_next]);
                    }
                }
            }
        }
    ...
    pub(crate) fn encode_weighted_agree_eq(
        ...
        for c_zw in 0..=zw_lits.len() {
            ...
            if rem < 0 || rem as usize > xy_lits.len() {
                continue;
            }
...
pub(crate) fn sat_encode(problem: Problem, tuple: SumTuple) -> (SatEncoder, radical::Solver) {
    ...
    let sum_values = |abs_s: i32, len: usize| -> Vec<u32> {
        if abs_s == 0 {
            ...
            [abs_s, -abs_s]
                .iter()
                .filter_map(|&s| sigma_full_to_cnt(s, 0, len))
                .collect()
        }
    };
...
pub(crate) fn sat_encode_quad_pb_unified(
    ...
    if z_fixed.is_none() {
        solver.add_clause([enc.z_var(0)]);
    }
    if w_fixed.is_none() {
        solver.add_clause([enc.w_var(0)]);
    }

    // Helper: check sum parity
    let check_sum = |sum: i32, len: usize| -> Option<u32> {
        if (sum + len as i32) % 2 != 0 {
            return None;
        }
    ...
    for (si, seq) in seqs.iter().enumerate() {
        match seq.fixed {
            ...
            Some(vals) => {
                ...
                if actual_sum != seq.sum {
                    return None;
                }
                // Fix variables
                for i in 0..seq.len {
                    solver.add_clause([if vals[i] > 0 {
                        seq_var(si, i)
                    } else {
                        -seq_var(si, i)
                    }]);
    ...
    for k in 1..n {
        ...
        for (si, seq) in seqs.iter().enumerate() {
            ...
            for i in 0..overlap {
                lits_a.push(seq_var(si, i));
                lits_b.push(seq_var(si, i + k));
                coeffs.push(w);
                lits_a.push(-seq_var(si, i));
                lits_b.push(-seq_var(si, i + k));
                coeffs.push(w);
