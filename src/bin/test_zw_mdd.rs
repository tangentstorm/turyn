...
fn main() {
    ...
    fn count_xy(
        ...
        let total: u128 = nodes[nid as usize]
            .iter()
            .map(|&child| count_xy(child, level + 1, depth, nodes, memo))
            .sum();
    ...
    fn walk_zw<F: FnMut(u32, u32, u32)>(
        ...
        for branch in 0u32..4 {
            ...
            if child == turyn::mdd_reorder::DEAD {
                continue;
            }
    ...
    walk_zw(
        ...
        &mut |z_bits, w_bits, xy_root| {
            zw_count += 1;
            let xy_count = if let Some(&c) = xy_counts.get(&xy_root) {
                c
            } else {
                let c = count_xy(
                    xy_root,
            ...
            if zw_count <= 10 {
                eprintln!(
                    "  z={:0width$b} w={:0width$b} → {} XY configs (node {})",
                    z_bits,
                    w_bits,
                    xy_count,
                    xy_root,
