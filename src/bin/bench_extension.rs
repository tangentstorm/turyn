...
fn main() {
    ...
    fn walk(
        ...
        if level == hd {
            if in_zw {
                walk(nid, 0, hd, z, w, 0, 0, false, po, nodes, out, limit);
            } else {
                out.push(Entry {
                    z_bits: z,
                    w_bits: w,
        ...
        let branches: Vec<(u32, u32)> = if nid == u32::MAX {
            ...
            (0..4u32)
                .filter(|&b| nodes[nid as usize][b as usize] != 0)
                .map(|b| (nodes[nid as usize][b as usize], b))
                .collect()
        };
        let pos = po[level];
    ...
    fn count_paths(nid: u32, level: usize, depth: usize, nodes: &[[u32; 4]]) -> u64 {
        ...
        if nid == u32::MAX {
            return 4u64.pow((depth - level) as u32);
        }
        (0..4)
            .map(|b| count_paths(nodes[nid as usize][b], level + 1, depth, nodes))
            .sum()
    }
    ...
    for base_k in [5, 7, 10] {
        ...
        let mdd = match mdd_reorder::Mdd4::load(&path) {
            Some(m) => m,
            None => {
                eprintln!("Skipping k={}: not found", base_k);
                continue;
            }
        };
        ...
        for extra in 1..=4 {
            ...
            eprintln!(
                "  k={}->{}:  {}/{} hits ({:.1}% pruned), avg={:.0}us/call, total_paths={}{}",
                base_k,
                target_k,
                hits,
                test_n,
        ...
        let test_n = entries.len().min(200);
        let dead1: Vec<bool> = entries
            .iter()
            .take(test_n)
            .map(|e| {
                let (_, r) = mdd_zw_first::build_extension(
                    base_k,
                    base_k + 1,
                    e.z_bits,
                    e.w_bits,
