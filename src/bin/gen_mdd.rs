...
fn main() {
    ...
    let reordered = if legacy {
        ...
        eprintln!(
            "Using sequential ZW-first builder{}...",
            if k >= 11 && !sequential {
                " (auto: k>=11 saves memory)"
            } else {
                ""
            }
        );
        ...
        let zw = if parallel_depth > 0 {
            eprintln!(
                "Using parallel ZW-first builder (depth={}, {} subtrees, {} cores)...",
                parallel_depth,
                4u32.pow(parallel_depth as u32),
                rayon::current_num_threads()
            );
            turyn::mdd_zw_first::ZwFirstMdd::build_parallel_depth(k, parallel_depth)
        } else {
            eprintln!(
                "Using parallel ZW-first builder ({} cores)...",
                rayon::current_num_threads()
            );
