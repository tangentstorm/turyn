...
fn main() {
    ...
    let outfile = args.get(2).map(|s| s.as_str()).unwrap_or(&default_out);
    ...
    let size_bytes = 16 + node_count * 16; // header + node data
