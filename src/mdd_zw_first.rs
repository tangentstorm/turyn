...
impl BuildStats {
    ...
    pub fn report(&self, k: usize) {
        ...
        eprintln!(
            "  Total: {} calls, {} hits ({:.1}%), {} pruned",
            total_zw_calls,
            total_zw_hits,
            if total_zw_calls > 0 {
                100.0 * total_zw_hits as f64 / total_zw_calls as f64
            } else {
                0.0
            },
            total_zw_pruned
        ...
        eprintln!(
            ...
            total_xy_hits,
            if total_xy_calls > 0 {
                100.0 * total_xy_hits as f64 / total_xy_calls as f64
            } else {
                0.0
            },
            total_xy_pruned
...
impl ZwFirstMdd {
    ...
    pub fn build_parallel_depth(k: usize, parallel_depth: usize) -> Self {
        ...
        eprintln!(
            "  Parallel build: {} subtrees (depth {}, {} cores)...",
            num_subtrees,
            parallel_depth,
        ...
        let subtree_branches: Vec<Vec<u32>> = (0..num_subtrees)
            .map(|idx| (0..parallel_depth).map(|d| (idx >> (d * 2)) & 3).collect())
            .collect();

        // Build each subtree independently in parallel
        let branch_results: Vec<(Self, Vec<u32>)> = subtree_branches
            .into_par_iter()
            .map(|branches| {
        ...
        eprintln!(
            "  Parallel build: all subtrees done in {:.1?}, merging...",
            start.elapsed()
        );
        ...
        for (mdd, branches) in &branch_results {
            ...
            let remapped_root = if nid == DEAD || nid == LEAF {
                nid
            } else {
                nid - 1 + offset
            };
        ...
        fn build_prefix_level(
            ...
            for b in 0u32..4 {
                path.push(b);
                children[b as usize] =
                    build_prefix_level(depth + 1, max_depth, path, subtree_map, nodes);
                path.pop();
            }
            // Reduce
            if children.iter().all(|&c| c == DEAD) {
        ...
        let level1_root = build_prefix_level(
            0,
            parallel_depth,
            &mut path,
            &subtree_map,
        ...
        eprintln!(
            "ZW-first MDD k={} (parallel): {} nodes, {:.1} MB, {:.1?}",
            k,
            merged_nodes.len(),
            merged_nodes.len() as f64 * 16.0 / 1_048_576.0,
            start.elapsed()
        );
    ...
    fn build_inner_opts(
        ...
        for j in 0..k {
            let xy_pairs: Vec<(usize, usize)> = (0..k - j).map(|i| (i, k + i + j)).collect();
        ...
        for (li, lag) in lags.iter().enumerate() {
            ...
            for &(a, b) in &lag.z_pairs {
                ...
                zw_events_at_level[complete].push((
                    li,
                    LagEvent {
                        pos_a: a,
                        pos_b: b,
            ...
            for &(a, b) in &lag.xy_pairs {
                let complete = pos_to_level[a].max(pos_to_level[b]);
                xy_events_at_level[complete].push((
                    li,
                    LagEvent {
                        pos_a: a,
                        pos_b: b,
        ...
        for (li, lag) in lags.iter().enumerate() {
            for &(a, b) in &lag.xy_pairs {
                let complete = pos_to_level[a].max(pos_to_level[b]);
                for level in 0..=complete {
                    xy_max_remaining[level][li] += 2;
                }
            }
        }

        // Active position tracking for z,w half
        let zw_pairs: Vec<Vec<(usize, usize)>> = zw_events_at_level
            .iter()
            .map(|evs| evs.iter().map(|(_, ev)| (ev.pos_a, ev.pos_b)).collect())
            .collect();
        let (zw_active_at_level, zw_active_indices) =
            compute_active_tracking(zw_depth, 2 * k, &pos_order, &zw_pairs);

        let xy_pairs: Vec<Vec<(usize, usize)>> = xy_events_at_level
            .iter()
            .map(|evs| evs.iter().map(|(_, ev)| (ev.pos_a, ev.pos_b)).collect())
            .collect();
        ...
        let zw_resolved_events: Vec<Vec<(usize, usize, usize, [i8; 16])>> = zw_events_at_level
            .iter()
            .enumerate()
            .map(|(level, evs)| {
                evs.iter()
            ...
            .collect();
        let xy_resolved_events: Vec<Vec<(usize, usize, usize, [i8; 16])>> = xy_events_at_level
            .iter()
            .enumerate()
            .map(|(level, evs)| {
                evs.iter()
        ...
        let canonical_rules =
            is_bouncing_order(&pos_order, k) && std::env::var("MDD_DISABLE_CANON").is_err();
        ...
        let mut zw_memo: Vec<HashMap<BuildMemoKey, u32>> =
            (0..=zw_depth).map(|_| HashMap::default()).collect();
        let mut xy_memo: Vec<HashMap<BuildMemoKey, u32>> =
            (0..=zw_depth).map(|_| HashMap::default()).collect();
        ...
        } else if restrict_level1.is_some() {
            4
        } else {
            1
        };
        ...
        let total_memo_cap: usize = std::env::var("MDD_MEMO_CAP")
            ...
            .unwrap_or(auto_cap);
        eprintln!(
            "  Memo budget: {} entries ({:.1} GB) across {} parallel tasks",
            total_memo_cap,
            total_memo_cap as f64 * 140.0 / 1e9,
            num_parallel
        );
        ...
        eprintln!(
            "ZW-first MDD k={}: {} nodes, {:.1} MB (zw_memo={}, xy_memo={}, xy_cache={}/{} = {:.1}% hit)",
            k,
            nodes.len(),
            ...
            xy_cache.len(),
            zw_boundary_calls,
            if zw_boundary_calls > 0 {
                100.0 * (1.0 - xy_cache.len() as f64 / zw_boundary_calls as f64)
            } else {
                0.0
            }
        );
    ...
    fn count_xy_rec(&self, nid: u32, level: usize, memo: &mut HashMap<u32, u128>) -> u128 {
        ...
        let total: u128 = self.nodes[nid as usize]
            .iter()
            .map(|&child| self.count_xy_rec(child, level + 1, memo))
            .sum();
...
pub fn build_zw_dfs(
    ...
    if level == ctx.depth {
        ...
        for li in 0..ctx.k {
            ...
            if zw_val.abs() > ctx.xy_max_abs[li] {
                return DEAD;
            }
            if (zw_val + ctx.xy_max_abs[li]) % 2 != 0 {
                return DEAD;
            }
        }
        if ctx.zw_only {
        ...
        if let Some(&cached) = xy_cache.get(&sums_key) {
            return cached;
        }

        for m in xy_memo.iter_mut() {
            m.clear();
        }
    ...
    for branch in 0u32..4 {
            ...
            && new_pos == ctx.w_tail_pos
        {
            ...
            if w_bit == 1 {
                new_rule_state |= BDKR_W_TAIL_BIT;
            }
        }
        ...
        if ok && level + 1 < ctx.depth {
            for li in 0..ctx.k {
                ...
                if (zw_val.abs() - zw_remaining) > ctx.xy_max_abs[li] {
                    ok = false;
                    break;
                }
            }
...
pub fn build_xy_dfs(
    ...
    for branch in 0u32..4 {
        ...
        if ctx.symmetry_break && new_pos == 2 * ctx.k - 1 && branch != 0b11 {
            continue;
        }
        current_vals[new_idx] = branch as u8;
...
pub fn build_xy_for_boundary_raw(k: usize, zw_sums: &[i8]) -> (Vec<[u32; 4]>, u32) {
    ...
    let mut xy_events_at_level: Vec<Vec<(usize, LagEvent)>> =
        (0..zw_depth).map(|_| Vec::new()).collect();
    let mut xy_lag_check_at_level: Vec<Vec<usize>> = (0..=zw_depth).map(|_| Vec::new()).collect();
    ...
    let xy_pairs_for_active: Vec<Vec<(usize, usize)>> = xy_events_at_level
        .iter()
        .map(|evs| evs.iter().map(|(_, ev)| (ev.pos_a, ev.pos_b)).collect())
        .collect();
    let (xy_active_at_level, xy_active_indices) =
    ...
    // Pre-resolve events
    let resolved_events: Vec<Vec<(usize, usize, usize, [i8; 16])>> = xy_events_at_level
        .iter()
        .enumerate()
        .map(|(level, evs)| {
            evs.iter()
    ...
    let mut unique: HashMap<u64, u32> = HashMap::default();
    let mut memo: Vec<HashMap<BuildMemoKey, u32>> =
        (0..=zw_depth).map(|_| HashMap::default()).collect();
...
pub fn build_extension(
    ...
    let classify = |pos: usize| -> Option<usize> {
        if pos < base_k {
            None
        } else if pos < target_k {
            Some(pos - base_k)
        } else if pos < target_k + extra {
            Some(extra + (pos - target_k))
        } else {
            None
        }
    };
    ...
    for j in 0..target_k {
        ...
        for i in 0..target_k.saturating_sub(j) {
            ...
            if ext_a.is_none() && ext_b.is_none() {
                continue;
            }
            ...
            max_complete_level =
                Some(max_complete_level.map_or(complete_level, |c: usize| c.max(complete_level)));
        }
        ...
        if j < target_k - 1 {
            for i in 0..target_k - j - 1 {
                ...
                events_at_level[complete_level].push(event);
                max_complete_level = Some(
                    max_complete_level.map_or(complete_level, |c: usize| c.max(complete_level)),
                );
            }
        }
    ...
    let combined_pairs: Vec<Vec<(usize, usize)>> = (0..ext_depth)
        .map(|l| {
    ...
    let zw_resolved: Vec<Vec<(usize, usize, usize, [i8; 16])>> = events_at_level
        .iter()
        .enumerate()
        .map(|(level, evs)| {
            evs.iter()
                .map(|ev| match &ev.kind {
                    ExtEventKind::NewNew { ext_a, ext_b, is_z } => (
                        ev.lag,
                        active_indices[level][*ext_a],
                        active_indices[level][*ext_b],
                        make_zw_delta(*is_z),
                    ),
                    ExtEventKind::OldNew {
                        ext_new,
                        ...
                        for a in 0u32..4 {
                            for b in 0u32..4 {
                                ...
                                delta[(a * 4 + b) as usize] = (2 * (*old_sign_z as i32) * nz
                                    + 2 * (*old_sign_w as i32) * nw)
                                    as i8;
                            }
                        }
    ...
    let xy_resolved: Vec<Vec<(usize, usize, usize, [i8; 16])>> = xy_events_at_level
        .iter()
        .enumerate()
        .map(|(level, evs)| {
            evs.iter()
                        ...
                        ExtEventKind::OldNew {
                            ...
                            for a in 0u32..4 {
                                for b in 0u32..4 {
                                    ...
                                    delta[(a * 4 + b) as usize] =
                                        ((*old_x as i32) * nx + (*old_y as i32) * ny) as i8;
                                }
                            }
                            (ev.lag, idx, idx, delta)
                        }
    ...
    let mut unique: HashMap<u64, u32> = HashMap::default();
    let mut zw_memo: Vec<HashMap<BuildMemoKey, u32>> =
        (0..=ext_depth).map(|_| HashMap::default()).collect();
    let mut xy_memo: Vec<HashMap<BuildMemoKey, u32>> =
        (0..=ext_depth).map(|_| HashMap::default()).collect();
    ...
    let (init_zw_state, init_xy_state) =
        match compute_extension_initial_rule_state(base_k, z_bits, w_bits, x_bits, y_bits) {
            Some(s) => s,
            None => {
                return (nodes, DEAD);
            }
    ...
    let canonical_rules =
        is_bouncing_order(&pos_order, extra) && std::env::var("MDD_DISABLE_CANON").is_err();
    ...
    if std::env::var("MDD_PROFILE").is_ok() {
        eprintln!(
            "build_extension: base_k={}, target_k={}, extra={}, {} nodes, root={}",
            base_k,
            target_k,
            extra,
            nodes.len(),
...
pub fn has_extension_fast(
    ...
    let zv = |p: usize| -> i32 {
        if p < base_k {
            if (z_bits >> p) & 1 == 1 { 1 } else { -1 }
        } else {
            let b = base_k + (p - (n - base_k));
            if (z_bits >> b) & 1 == 1 { 1 } else { -1 }
        }
    };
    let wv = |p: usize| -> i32 {
        if p < base_k {
            if (w_bits >> p) & 1 == 1 { 1 } else { -1 }
        } else {
            let b = base_k + (p - (m - base_k));
            if (w_bits >> b) & 1 == 1 { 1 } else { -1 }
        }
    };
    let xv = |p: usize| -> i32 {
        if p < base_k {
            if (x_bits >> p) & 1 == 1 { 1 } else { -1 }
        } else {
            let b = base_k + (p - (n - base_k));
            if (x_bits >> b) & 1 == 1 { 1 } else { -1 }
        }
    };
    let yv = |p: usize| -> i32 {
        if p < base_k {
            if (y_bits >> p) & 1 == 1 { 1 } else { -1 }
        } else {
            let b = base_k + (p - (n - base_k));
            if (y_bits >> b) & 1 == 1 { 1 } else { -1 }
        }
    };
    ...
    for &j in &check_lags {
        ...
        for i in 0..n.saturating_sub(j) {
            let a = i;
            let b = i + j;
            if !is_z_bnd(a) || !is_z_bnd(b) {
                continue;
            }
            let a_old = is_old_z(a);
            let b_old = is_old_z(b);
            let a_new_idx = if a == z_new[0] {
                Some(0usize)
            } else if a == z_new[1] {
                Some(1)
            } else {
        ...
        for i in 0..m.saturating_sub(j) {
            let a = i;
            let b = i + j;
            if !is_w_bnd(a) || !is_w_bnd(b) {
                continue;
            }
            let a_old = is_old_w(a);
            let b_old = is_old_w(b);
            let a_new_idx = if a == w_new[0] {
                Some(2usize)
            } else if a == w_new[1] {
                Some(3)
            } else {
        ...
        let z_bnd: i32 = (0..n.saturating_sub(j))
            .filter(|&i| is_z_bnd(i) && is_z_bnd(i + j))
            .count() as i32;
        let w_total = m.saturating_sub(j) as i32;
        let w_bnd: i32 = (0..m.saturating_sub(j))
            .filter(|&i| is_w_bnd(i) && is_w_bnd(i + j))
            .count() as i32;
        let max_mid =
            2 * (z_total - z_bnd) + 2 * (w_total - w_bnd) + (z_total - z_bnd) + (z_total - z_bnd); // X + Y same as Z

        lag_infos.push(LagInfo {
            old_old_sum,
            max_mid,
            linear,
            bilinear,
...
pub fn has_zw_extension_fast(base_k: usize, n: usize, z_bits: u32, w_bits: u32) -> bool {
    let m = n - 1;
    let tk = base_k + 1;
