...
pub(crate) struct SearchConfig {
    ...
    /// XY product law: for 1 <= i <= n with U_i := x_i * y_i, enforce
    /// U_1 = +1, U_n = +1, and U_i = -U_{n+1-i} for 2 <= i <= n-1.
    /// Endpoint pins already hold from rule (i)
    /// (x_0=y_0=x_{n-1}=y_{n-1}=+1), so only the middle pairwise
    /// equalities are added. Implies `X . Y = 2`.
    ///
    /// This is a **theorem**, not a conjecture: `Turyn.xy_product_law`
    /// in `lean/Turyn/XY.lean` proves it for every `n >= 4` under
    /// `Canonical1` (BDKR rule (i)), which every producer enforces. It
    /// is therefore on by default and does NOT taint the TTC quality
    /// label. `--no-xy-product` disables it for A/B benchmarking.
