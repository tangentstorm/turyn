...
fn boundary_key_from_values(values: &[i8], k: usize) -> u128 {
...
fn build_zw_candidates(
    ...
    let mut z_buckets: HashMap<u128, Vec<SeqWithSpectrum>> = HashMap::new();
    generate_sequences_with_sum_visit(problem.n, tuple.z, true, true, cfg.max_z, |values| {
        ...
        if let Some(spectrum) = spectrum_if_ok(&values_f64, spectral_z, problem.spectral_bound()) {
            ...
            z_buckets
                .entry(boundary_key_from_values(values, cfg.boundary_k))
                .or_default()
    ...
    let mut w_buckets: HashMap<u128, Vec<SeqWithSpectrum>> = HashMap::new();
